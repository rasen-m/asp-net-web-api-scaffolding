...
if (!$foundDbContextType) {
	...
	if ($DbContextType.Contains(".")) {
		...
		$outputPath = Join-Path DataAccessLayer $DbContextType
		if ($Area) {
	...
	Add-ProjectItemViaTemplate $outputPath -Template DbContextTemplate -Model @{
...
if ($foundDbContextType) {
	...
	if (!$isDbContext) {
		...
		Add-ClassMemberViaTemplate -Name $propertyName -CodeClass $foundDbContextType -Template DbContextEntityMemberTemplate -Model @{
